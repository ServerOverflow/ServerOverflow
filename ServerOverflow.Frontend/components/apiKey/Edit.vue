<template>
  <dialog ref="dialog" class="modal modal-bottom sm:modal-middle">
    <div class="modal-box flex flex-col">
      ...
      <h3 class="text-lg font-bold">Edit an API key</h3>
      <div class="divider divider-start m-0 mb-2"/>
      <div class="overflow-y-auto flex-1 pr-4 -mr-4">
        <ApiKeyForm :submit="submit" submit-label="Save changes" :api-key="apiKey"/>
      </div>
    </div>
...
async function submit(data, node) {
  ...
  try {
    await $axios.post(`/key/${apiKey.value.id}`, data);
    toast.add({
      title: `Successfully modified an API key`,
      color: 'success'
    });
    close();
