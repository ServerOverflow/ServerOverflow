<template>
  <dialog ref="dialog" class="modal modal-bottom sm:modal-middle">
    <div class="modal-box flex flex-col">
      ...
      <h3 class="text-lg font-bold">Create a new invitation</h3>
      <div class="divider divider-start m-0 mb-2"/>
      <div class="overflow-y-auto flex-1 pr-4 -mr-4">
        <InvitationForm :submit="submit" submit-label="Create"/>
      </div>
    </div>
...
async function submit(data, node) {
  ...
  try {
    await $axios.post('/invitation/create', data);
    toast.add({
      title: `Successfully created a new invitation`,
      color: 'success'
    });
    close();
